...
def generate_network(batchSize):
    ...
    #w_multiple = tf.constant(value=1.0, shape=[ 3, 3], dtype=tf.float32)

    #b_multiple = tf.constant(value=1.0, shape=[ 1, 3], dtype=tf.float32)

    #we_multiple = tf.constant(value=1.0, shape=[ 1, 3], dtype=tf.float32)

    #be_multiple = tf.constant(value=1.0, shape=[1, 1], dtype=tf.float32)
    ...
    input_layer = tf.placeholder(dtype=tf.float32, shape= [None,1,3], name= "input_layer")

    input_layer_base = tf.placeholder(dtype=tf.float32, shape= [None,1,3], name= "input_layer_base")
...
expected_output = tf.placeholder(dtype= tf.float32, shape= [None,1,1], name= "expected_output")
...
gradientDescent = tf.train.AdamOptimizer().minimize(network_cost, var_list=[w_1, b_1,w_2,b_2,w_3,b_3])
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
    ...
    train_data = generate_training_data(1000, 10000)
    ...
    summary_writer = tf.summary.FileWriter("/home/sreramk/PycharmProjects/neuralwithtensorgpu/visual/", graph=tf.get_default_graph())

    # Runs the op.

    for i in range(len(train_data)):
        __, summary = sess.run(fetches=[gradientDescent, network_cost_scalar], feed_dict={input_layer : train_data[i][0],
